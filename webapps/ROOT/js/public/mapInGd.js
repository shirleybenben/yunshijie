	...
	$.fn.GdMap = function(options){
	    ...
	    _this.showCarInfo = function(map){
			var lnglats= options.okey,
				...
				infoWindow = new AMap.InfoWindow({offset:new AMap.Pixel(-2,-20)}),
			...
			for(var i=0;i< lnglats.length;i++){
				...
				if(i>1){
					switch(exceptions[i-2][0]){
						case "01":
							markers[i].icon = "img/orange.png";
							markers[i].exception = "01"
						break;
						default: 
							markers[i].icon = "img/blue.png";
					...
					if(lnglats.length<3){
						switch(i){
							case 0:
								markers[i].icon = "img/end_point.png"
							break;
							case 1:
								markers[i].icon = "img/start.png"
							break;
						}
					}else{
						switch(i){
			...
			for(var j=0;j< markers.length;j++){
				...
				if(j>1){
					if(options.speed[j] === null || options.speed[j] === undefined){
						...
						curSped = options.speed[j]+"km/h";
					}
					if(markers[j].exception == "01"){
				        //构建信息窗体中显示的内容
						info.push('<div class="normal unnormal">');
						info.push('<div class="tit clearfix">');
						info.push('<span class="tit_l">货源编号：'+options.sourceNo+'</span></div>');
						info.push('<div class="con clearfix"><div class="con_l"></div><div class="con_r">');
						info.push('<span>'+options.carNo+'</span>');
						info.push('<span>行驶速度：'+curSped+'</span>');
						info.push('<span>异常类型：<em>'+options.exceptionName[j-2]+'</em></span>');
						info.push('<span>异常开始时间：'+options.expSTime[j-2]+'</span>');
						info.push('<span>异常结束时间：'+options.expETime[j-2]+'</span>');
						info.push('<span>位置：'+markers[j].position[1]+","+ markers[j].position[0]+'</span></div></div></div>');
					}else{
				        //构建信息窗体中显示的内容
				        info.push('<div class="normal"><div class="tit clearfix">');
				        info.push('<span class="tit_l">货源编号：'+options.sourceNo+'</span></div>');
				        info.push('<div class="con clearfix"><div class="con_l"></div>');
				        info.push('<div class="con_r"><span>'+options.carNo+'</span>');
				        info.push('<span>行驶速度：'+curSped+'</span>');
				        info.push('<span>获取时间：'+options.expSTime[j-2] +'</span>');
				        info.push('<span>位置：'+markers[j].position[1]+','+ markers[j].position[0]+'</span></div>');
				        info.push('</div>');
					}
				}else{
					//设置起点终点的信息
					...
					info.push("<div class=\"normal orgin\"><p>"+address+"</p>");
				}
...
var marker,infoWindow= new AMap.InfoWindow({offset:new AMap.Pixel(0,-35)});
