...
var global = $.extend(global_config, {
    ...
    "getUserId": function () {
        return global.getUser().accountId;
    },
...
global.remindLogin = function () {
    ...
    if (!global.isLogin() && currentPage!="index.htm") {
        location.href =global.getContextPath() + '/login_dialog.htm';
    }
}
...
global.Logout = function(){
    ...
    $.post(global.server + url,data,function(msg){
        if (msg && msg.status && msg.status.statusCode == global.status.success) {
            global.logout();
            var url = global.getContextPath() + "/login_dialog.htm";
            setTimeout('location.href="'+ url+'"', 50);
        }
    })
};
...
$(function(){
    ...
    // 返回顶部
    $("<a href='javascript:;' class='toTop'></a>").appendTo('body').toTop({
        autohide: true,
        offset: 420,
        speed: 200,
        position: true,
